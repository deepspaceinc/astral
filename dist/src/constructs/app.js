...
import { LocalWorkspace } from '../utils/pulumi-automation.js';
...
class App {
     ...
     */
    async deploy(options, setDeployList) {
    ...
    getPulumiProgram() {
        return async () => {
            ...
            const bucketPolicy = new aws.s3.BucketPolicy("logBucketPolicy", {
                bucket: logBucket.id,
                policy: pulumiLib.all([logBucket.arn, region, accountId]).apply(([bucketArn, region, accountId]) => JSON.stringify({
                    Version: "2012-10-17",
                    Statement: [
                        {
                            Effect: "Allow",
     ...
     */
    async deployECS(options, setDeployList) {
        try {
            ...
            if (options?.dryrun) {
                ...
                return parseDryrun(preview.stdout);
            }
